...
import { createEmptySudokuGrid } from '@/models/SudokuGrid';
import { SudokuValueGrid, SudokuPossibleValuesGrid } from '@/components/SudokuGrids';
import { initUltilities } from '@/ultilities/initUltilities';
import { processUltilities } from '@/ultilities/process/processUltilities';
...
export default function HomePage() {
  ...
  const handleValidate = () => { setMessage(getValidationMessage()); };
  ...
  const handleRandom = (difficulty: 'easy' | 'medium' | 'hard') => {
    let values = initUltilities.random(difficulty);
  ...
  const handleRandomEasy = () => { handleRandom('easy');};
  const handleRandomMedium = () => { handleRandom('medium'); };
  const handleRandomHard = () => { handleRandom('hard'); };
  const handleBlockInit = () => { !grid.isBlockedInit && setGrid(grid.blockInit()); };
  const handleReset = () => { setGrid(grid.reset()); };
  ...
  const selectedTechniqueOptions = techniqueOptions.filter(
    (option) => option.value !== SELECT_ALL_VALUE && selectedTechniques.includes(option.value as Technique)
  );
  ...
  const handleProcess = () => {
    ...
    const processedResult = processUltilities.process(grid, selectedTechniques, translations);
    ...
    <main className="min-h-screen items-center justify-center bg-gradient-to-br from-blue-100 via-white to-pink-100">
      <section className="w-full shadow-xl bg-white border border-gray-200 pl-6">
        ...
        <div className="mt-3 mb-3">
          <span className="flex gap-4">
            <span className="label-interaction">
              <h2 className="text-2xl font-bold tracking-tight drop-shadow">{translations.interaction.initTitle}</h2>
            </span>
            <button className="btn-interaction" title={translations.interaction.randomEasy} onClick={handleRandomEasy} >
              {translations.interaction.randomEasy}
            </button>
            <button className="btn-interaction" title={translations.interaction.randomMedium} onClick={handleRandomMedium}>{translations.interaction.randomMedium}</button>
            <button className="btn-interaction" title={translations.interaction.randomHard} onClick={handleRandomHard}>{translations.interaction.randomHard}</button>
            <button
        ...
        <div className="mb-3">
            <span className="flex gap-4">
              <span className="label-interaction">
              <h2 className="text-2xl font-bold tracking-tight drop-shadow">{translations.interaction.functionalTitle}</h2>
            </span>
            <button className="btn-interaction" title={translations.interaction.blockInit} onClick={handleBlockInit}>{translations.interaction.blockInit}</button>
              <button className="btn-interaction" title={translations.interaction.reset} onClick={handleReset}>{translations.interaction.reset}</button>
            <button className="btn-interaction" title={translations.interaction.validate} onClick={handleValidate}>{translations.interaction.validate}</button>
          </span>
        </div>
        {/* Solve Block */}
        <div className="mb-3">
          <span className="flex gap-4">
            <span className="label-interaction">
              <h2 className="text-2xl font-bold tracking-tight drop-shadow">{translations.interaction.techniquesTitle}</h2>
            </span>
            ...
            <button className="btn-interaction" onClick={handleProcess}>{translations.interaction.process}</button>
          </span>
        </div>
      </section>
      <section className="w-full shadow-xl bg-white border border-gray-200 pl-6">
        ...
        <div className="flex flex-col md:flex-row gap-8 mt-8 mb-8 items-start justify-center">
          ...
          <div>
            ...
            <SudokuPossibleValuesGrid grid={grid} 
