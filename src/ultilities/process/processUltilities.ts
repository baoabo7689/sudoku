...
import { doBruteForce } from "@/ultilities/process/bruteForce";
...
function deepCopyGrid(grid: SudokuGrid): SudokuGrid {
  return {
    ...grid,
    blocks: grid.blocks.map(blockRow => blockRow.map(block => ({
      ...block,
...
function process(
  ...
  const techniqueNames = selectedTechniques.map((technique) => translations.techniques[technique] ?? technique);
  const processedMessage = translations.processing.processedWithTechniques.replace('{techniques}', techniqueNames.join(', '));
...
export const processUltilities = {
  process
};
