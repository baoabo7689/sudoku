...
function canPlaceValue(grid: SudokuGrid, targetRow: number, targetCol: number, value: number): boolean {
...
function markScanningCrossHatching(grid: SudokuGrid, cell: SudokuCell, globalRow: number, globalCol: number) : boolean{
  let isUnchanged = true;
  
  const valueToMark = cell.solvedValue!;
  for (var col = 0; col < 9; col++) {
    if (col === globalCol) { continue; }
...
function preScanningCrossHatching(grid: SudokuGrid): boolean {
  let isUnchanged = true;
...
export function doScanningCrossHatching(grid: SudokuGrid): boolean {
  let isUnchanged = preScanningCrossHatching(grid);

  for (let value = 1; value <= 9; value++) {    
    for (let globalRow = 0; globalRow < 9; globalRow++) {
      let position: SudokuCell | null = null;
      let positionCol = -1;
  ...
  for (let value = 1; value <= 9; value++) {    
    for(let blockRow = 0; blockRow < 3; blockRow++) {
      for(let blockCol = 0; blockCol < 3; blockCol++) {
        ...
        if (!shouldSkip && matchCount == 1 && position && !position.solvedValue && canPlaceValue(grid, positionGlobalRow, positionGlobalCol, value)) {
          position.addSolvedValue(value);
          isUnchanged = false;
        }
      }
